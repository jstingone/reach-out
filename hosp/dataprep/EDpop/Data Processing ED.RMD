...
```{r}
insight_covid_encounters_nevi = read_sas("L:/dcore-prj0131-SHARED/dcore-prj0131-Stingone/ED_Dataset/Rerun/insight_covid_encounters_nevi.sas7bdat")
...
km_df = insight_covid_encounters_nevi %>% 
  ...
  mutate(death = case_when(discharge_disposition == "Expired" ~ 1,
         ...
         hosp = case_when(encounter_type == "Inpatient from Emergency" ~ 1,
                          encounter_type == "Emergency Only" ~ 0 ),
         nevi = nevi * 10,
         ...
         bc_avg = bc_avg * 10) %>%
  dplyr:: select(death, hosp, hospital_days, pm_avg, bc_avg, no2_avg, o3_avg,
                 nevi, score_demo, score_economic, score_residential, score_healthstatus,
                 age, sex, admit_date_phase, bmi_num, smoke, race, hispanic, address_zip, hosp, asthma, hyper, diabetes, hosp) %>%
  na.omit() %>%  
